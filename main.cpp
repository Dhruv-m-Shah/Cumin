		...
		try
		{
			...
			do
			{
				...
				for (long long i = 0; i <= BUFFER_LEN; i++) {
					if (buffer[i] == ' ') {
						contentLength = ConvertToInt1(contentLengthStr);
						contentLengthStr = "";
						for (long long j = i+1; j < i+1+contentLength; j++) {
							contentLengthStr += buffer[j];
						}
						break;
					}
					contentLengthStr += buffer[i];
				}

				cout << contentLengthStr << endl;
